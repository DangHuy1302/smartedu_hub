...
class _PomodoroScreenState extends State<PomodoroScreen> {
    ...
    if (start) {
      ...
      if (uid != null)
        _pomodoroService.pausePomodoro(
            uid: uid, remainingSeconds: _secondsRemaining);
    }
  }
  ...
  void _syncProgress() {
    ...
    if (uid != null && _isRunning)
      _pomodoroService.syncRemainingTime(
          uid: uid, remainingSeconds: _secondsRemaining);
  }
  ...
  void _handleTimerComplete() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
            'Bạn đã hoàn thành phiên học xuất sắc. Nhận điểm ngay thôi!'),
        actions: [
          ElevatedButton(
            onPressed: () {
    ...
    try {
      ...
      if (mounted) {
        ...
        if (isOnlineMode && bookingWasReleased) {
          msg +=
              ' Hệ thống đã giải phóng chỗ ngồi cho bạn tại $_currentRoomName.';
        } else {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Lỗi: Không thể lưu kết quả học tập.')));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
            isOnline ? 'Phòng học: $_currentRoomName' : 'Chế độ tự học Offline',
  ...
  Widget _buildStatusHeader() {
    String statusText = _state == PomodoroState.focusing
        ? 'Đang tập trung học tập'
        : 'Đã tạm dừng';
    Color statusColor =
        _state == PomodoroState.focusing ? Colors.green : Colors.orange;
    return Column(
      children: [
        Text(statusText,
            style: TextStyle(
                fontSize: 24, fontWeight: FontWeight.bold, color: statusColor)),
        if (_isRunning)
          const Padding(
              padding: EdgeInsets.only(top: 8),
              child: Text('Giữ màn hình luôn bật để tập trung tốt nhất',
    ...
    return Row(
      ...
      children: [
        ElevatedButton.icon(
          ...
          style: ElevatedButton.styleFrom(
              backgroundColor:
                  _isRunning ? Colors.orange : const Color(0xFF1565C0),
  ...
  Widget _buildBottomActions() {
    return Row(mainAxisAlignment: MainAxisAlignment.center, children: [
      TextButton.icon(
          ...
          label: const Text('Kết thúc & Nhận điểm',
              style: TextStyle(
          ...
          Expanded(
            child: StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                if (members.isEmpty)
                  return const Center(
                      child: Text('Không có ai khác trong phòng',
                          style: TextStyle(color: Colors.grey)));
                return ListView.separated(
                  ...
                  itemBuilder: (ctx, i) {
                    final m = members[i];
                    final String name =
                        m['fullName'] ?? m['email']?.split('@')[0] ?? 'Bạn học';
                    final String status = m['status'] ?? 'paused';
                    final int points = (m['studyPoints'] ?? 0) as int;
                    Color statusColor = status == 'focusing'
                        ? Colors.green
                        : (status == 'paused' ? Colors.orange : Colors.grey);
                    return _memberTile(
                        name,
                        status == 'focusing' ? 'Đang học' : 'Tạm nghỉ',
                        statusColor,
                        points);
                  },
                );
  ...
  Widget _memberTile(
      String name, String status, Color statusColor, int points) {
    return ListTile(
